...
tmux send-keys -t "$SESSION":0 "docker compose -f $COMPOSE_FILE up" C-m

# Window 1 → comrade-bot logs
tmux new-window -t "$SESSION":1 -n "comrade-bot"
tmux send-keys -t "$SESSION":1 "docker compose -f $COMPOSE_FILE logs -f comrade-bot" C-m

# Window 2 → politburo logs
tmux new-window -t "$SESSION":2 -n "politburo"
tmux send-keys -t "$SESSION":2 "docker compose -f $COMPOSE_FILE logs -f politburo" C-m

# Window 3 → vizburo logs
tmux new-window -t "$SESSION":3 -n "vizburo"
tmux send-keys -t "$SESSION":3 "docker compose -f $COMPOSE_FILE logs -f vizburo" C-m
